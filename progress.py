...
tab = functions.Window("Progress", "600x400")
tab.set_window()
    ...
    with open("data.json", "r") as file:
        ...
        if data != newdata:
            ...
            for orderID in range(len(newdata['orders'])):
                ...
                frame, label = tab.create_frame(rowspan=1, columnspan=5, row=orderID+1, column=0,
                                isOrder=True,
                                ...
                                isProgress=True, buttonText="Fulfill", buttonrow=2,
                                buttoncolumn=4)
                ...
                if newdata.get(f"{order_id} Progress") == "Completed":
                    ...
                    del data[f"{order_id} Progress"]
                    data['orders'].remove(order_id)
                    with open("data.json", "w") as write_file:
                        json.dump(newdata, write_file, indent=2)
                else:   
